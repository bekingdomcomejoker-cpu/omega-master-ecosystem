...
async def add_comment_to_marked(
    ...
    """Add a comment to a marked WhatsApp message"""
    try:
...
async def get_comments_on_marked(
    ...
    """Get all comments on a marked WhatsApp message"""
    try:
        thread = analyzer.comment_system.get_comments_for_item(message_id, "whatsapp")
        return thread.dict()
    except Exception as e:
        logger.error(f"[ERROR] {e}")
        raise HTTPException(status_code=500, detail=str(e))
